...
- 2026-09-09 进一步确认：WARP `warp` 全隧道可在服务器本机通过 IPv4、IPv6 和 UDP 自检，但会接管主机防火墙并阻断校外 SSH 与代理入站。因此“出口健康”不等于节点可用，不能在宿主机主网络直接常驻全隧道。
...
该路线目前只完成 TCP 端到端技术连通性验证。全隧道本机出口与 UDP 自检也已通过，但外部回连失败，尚不能交付为全流量节点。正式使用前仍需完成校园端回连、校园账户计费 A/B、地址变化和长期稳定性测试。需要全 TCP/UDP 时，不应直接开启服务器 UDP 后回退到校园 IPv4 出口。
